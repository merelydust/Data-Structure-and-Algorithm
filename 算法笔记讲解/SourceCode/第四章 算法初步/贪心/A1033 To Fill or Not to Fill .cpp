...
int main() {
    ...
    stations[n].distance = aimDistance; stations[n].price = 0; // 把终点也看作一个加油站
    ...
    while (now < n) {
        int minPriceIndex = -1; double minPrice = 100000; // 每次循环都选择前方加油站里最便宜的加油站
        for (int i = now+1; i <= n && stations[i].distance-stations[now].distance <= FULLRUN; ++i) { // within FULLRUN 找到第一个油价低于当前加油站的加油站 如果没有 就高个里挑最矮
            if (stations[i].price < minPrice) {
                minPrice = stations[i].price; minPriceIndex = i;
                if (minPrice < stations[now].price) break; // 比now还便宜 马上去
            }
        }
